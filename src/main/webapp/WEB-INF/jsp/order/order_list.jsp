...
<div class="pd-20">
    ...
    <span class="l">
    <a href="order?_type=toAdd" class="btn btn-primary radius"><i
            class="icon-plus"></i> 添加订单</a></span>
        <%--${fn:length(orderList)} 用于获取传入列表的长度--%>
        ...
        <c:forEach items="${orderList}" var="order" varStatus="status">
            <tr class="text-c">
                ...
                <td><span class="l" style="display: flex; justify-content: center;">
                    <form style="margin-right: 10px; display: inline-block;"
                          action="<c:url value="/order?_type=toEdit"/>" method="post">
                        <input type="hidden" id="editid" name="editid" value="${order.order_id}">
                <input type="submit" value="修改" class="btn btn-primary radius">
                        </form>
                    <form style="display: inline-block;" action="<c:url value="/order?_type=removeById"/>"
                          method="post">
                         <input type="hidden" name="deleteid" value="${order.order_id}">
